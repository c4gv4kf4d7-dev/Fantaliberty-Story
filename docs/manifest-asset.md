...
| File | Scena | Uso |
|---|---|---|
| `bg_esterno_vialetto` | S0 | Primo frame assoluto del gioco, prima di qualunque interazione. **Attenzione**: esistono due versioni di questo esterno e di quello dell'ingresso — quella buona e' il ridisegno consegnato il 30 agosto (vialetto 941x1672, ingresso 852x1846). Un giro di ripristino ha rimesso per sbaglio la versione vecchia, piu' piatta e a blocchi: prima di sostituirli, guardarli |
| `bg_esterno_ingresso` | S0 | L'ingresso: dietro Lucas che accoglie, e dietro il terminale della registrazione. **Il file nel repo e' ritoccato**: nel disegno originale c'era un figurante col tablet in mezzo alla porta, identico a Lucas, e in scena sembravano due. E' stato tolto ricostruendo il fondo. Se un domani l'immagine viene reimportata dal sorgente, torna anche lui |
| `bg_macintosh` | S0 | Il banco della registrazione col Mac gia' dentro l'immagine: e' il fondale di registrazione e badge (li' sfocato). Il terminale non e' disegnato — lo incolla il motore sopra il vetro del CRT, in pixel (`SCHERMO_FONDALE` in engine.js). Se l'immagine viene ridisegnata, quelle quattro misure vanno rifatte. `prop_mac_terminale` resta nel repo ma non lo usa piu' nessuno. Dentro il vetro, prima del terminale, ci va `prop_mac_hello` |
| `prop_mac_hello` | S0 | La schermata con cui il Mac e' acceso quando comincia la registrazione: MacPaint col "hello." scritto a mano, l'immagine con cui il Macintosh si e' presentato nel 1984. Non e' un oggetto in scena — sta dentro il vetro del CRT, nello stesso riquadro del terminale, e glielo cede appena nome e cognome sono dentro (step `prop` con `"schermata"`) |
...
| `bg_staff_door_locked` | S1 | Zona 5, la porta STAFF ONLY prima dello sblocco: porta a vetri moderna, coerente con lo Steve Jobs Theater, completamente chiusa, targhetta STAFF ONLY, lettore badge visibile in stato **neutro/rosso**, nessuna persona, spazio per il box del dialogo |
| `bg_staff_door_authorized` | S1 | Stessa inquadratura esatta della precedente (stessa architettura, stessa camera, stessa targhetta): cambia solo il lettore badge, in stato **autorizzato/verde**, e l'ambiente leggermente più luminoso. **La porta resta chiusa**: il badge è autorizzato, non la porta aperta fisicamente |
| `bg_campus_run_corridor` | S9 | Il corridoio interno, moderno e riservato, che si mostra dopo l'autorizzazione: la scena di ingresso al minigioco, subito prima che si apra la corsa sopra di esso. **In coda** (`meta.assetiInArrivo`) |
...
| `run_banner_icona` | I tre anelli dell'attivita' accanto al punteggio nella targa, che girano piano (consegnata come `banner_icon.png`). E' l'unico disegno tondo e liscio dell'interfaccia: gli altri comandi — l'ingranaggio del menu — sono a quadretti e disegnati dal codice |
| `run_avatar` | Il ritratto nella targa in alto a sinistra — **tre facce in fila**: neutra, dolorante (quando si prende una botta), soddisfatta (quando si recupera un cuore). E' l'unico posto in cui si vede la faccia del personaggio: le tre tavole della corsa sono tutte di spalle |
